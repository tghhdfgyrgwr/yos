...
let userData = await users.findOne({id: params.id, guildID: params.guild})    
...
if(userData.xp_voice === 0) userData.xp_voice = 1
if(userData.xp === 0) userData.xp = 1

let nowxp = reward_xp_voice.find(d => d.xp < userData.xp_voice).xp
let nextxp = reward_xp_voice.filter(d => d.xp > userData.xp_voice)[reward_xp_voice.filter(d => d.xp > userData.xp_voice).length - 1].xp 
    let lastxp = reward_xp_voice.find(d => d.xp < userData.xp_voice).xp
    let loadxp = userData.xp_voice - lastxp
    let xp316 = (nextxp - lastxp)/316
    let loading = loadxp / xp316
    
let nowxpchat = reward_xp.find(d => d.xp < userData.xp).xp
let nextxpchat = reward_xp.filter(d => d.xp > userData.xp)[reward_xp.filter(d => d.xp > userData.xp).length - 1].xp 
    let lastxpchat = reward_xp.find(d => d.xp < userData.xp).xp
    let loadxpchat = userData.xp - lastxpchat
    let xp316chat = (nextxpchat - lastxpchat)/316
    let loadingchat = loadxpchat / xp316chat
    
let chatlevel = reward_xp.find(d => d.xp < userData.xp).level
let voicelevel = reward_xp_voice.find(d => d.xp < userData.xp_voice).level
...
let data_voice = await users.find({guildID: params.guild, id: params.id}).select({xp_voice: 1})
...
let data_chat = await users.find({guildID: params.guild}).select({xp: 1})
...
ctx.textAlign = "center";
ctx.fontSize = '72px';
ctx.fillText(`${userData.xp}`, 745, 194, 997)
...
ctx.textAlign = "center";
ctx.fontSize = '72px';
ctx.fillText(`${userData.xp_voice}`, 745, 276, 997)
...
ctx.fillText(`${ontop_chat}`, 532, 138, 997)
   ...
   const loadtext = await Canvas.loadImage('https://cdn.discordapp.com/attachments/832995252621017098/840734504766144553/unknown.png');
    ...
    let xp =  reward_xp.filter(d => d.xp < userData.xp_voice)[reward_xp.filter(d => d.xp < userData.xp_voice).length - 1] - reward_xp.filter(d => d.xp > userData.xp_voice)[reward_xp.filter(d => d.xp > userData.xp_voice).length - 1].xp 
